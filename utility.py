...
async def get_table_names(orgn, kbs, uid="") -> list[str]:
    """Returns table names asynchronously."""
    return await asyncio.to_thread(lambda: [
        remove_special_characters(f"private_{uid}" if kb == "private" else f"{orgn}_{kb}")
        for kb in kbs
    ])
